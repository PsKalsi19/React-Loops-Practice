...
Each object has the following properties: id, name, ingredients, 
and instructions. Write a React component where you can create 
an array of recipes and generate an ordered list of recipes, where 
each list item displays the recipe's name, ingredients, and instructions. 
Data has been provided below for this questions. */
const recipes = [
  {
    id: 1,
        ...
        {recipes.map((ele) => {
          return (
            <li key={ele.id}>
              ...
              <ul>
                {ele.ingredients.map((ingredients, index) => (
                  <span>
                    {ingredients}
                    {ele.ingredients.length - 1 !== index ? ", " : "."}
                  </span>
                ))}
                <br />
                <p
                ...
                >
                  And this is the <strong>Recipe</strong> to Follow:
                </p>
                {ele.instructions.map((instructions) => (
                  <p>{instructions}</p>
                ))}
              </ul>
            </li>
